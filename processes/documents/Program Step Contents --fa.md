...
- پس از رسیدن مجموع ساعت فعالیت به ۱۵ ساعت، شما میبایست یک جلسه فنی با یک همیارفنی هماهنگ بکنید تا گزارش فعالیتتون توسط همیارفنی بررسی شود. این جلسات به‌صورت هفتگی توسط منتورهای فنی برگزار می‌شوند.
...
۲. **ایجاد حساب‌های کاربری در شبکه‌های اجتماعی**  
   ...
   در این پست‌ها، اینترن‌ها میتوانند توضیح دهند که از این شبکه‌های اجتماعی به‌عنوان ابزارهای ارتباطی با اعضای برنامه استفاده خواهد شد و می‌توانند اعلام کنند که قصد دارند فعالیت‌های خود را در برنامه CS Internhisp از طریق این پلتفرم‌ها به اشتراک بگذارند.  
 ...
 **نکته:** همیار فنی نقش "معلم" یا "ناظر رسمی" ندارد؛ بلکه همراهی است که در گذشته در جایگاه مشابهی بوده و اکنون در استپ‌های بالاتر فعالیت می‌کند و کارش همیار بودن فقط در این مسیر هست.
...
### تسک گفت‌وگو با منتور فنی

در انتهای هر استپ، تسکی به نام "جلسه با منتور فنی" برای اینترن تعریف شده است. این جلسه به‌منظور بررسی عملکرد کلی اینترن در طول استپ است. منتور فنی در این جلسه، گزارش‌ها و پست‌های اینترن را مرور می‌کند و سپس فیدبک خود را در پروفایل او ثبت می‌نماید.
...
در صورتی که اینترن **بیش از ۱۵ ساعت فعالیت** در برنامه داشته باشد، موظف است در **یک جلسه فنی** شرکت کند. این جلسات یک‌ساعته هستند و ظرفیت هر جلسه حداکثر چهار نفر است.
...
- وارد چت شخصی با **CS Club Bot** شوید و دکمه start بات را بزنید. (یا اگه دکمه start نیست، دستور /start را وارد کنید.)
...
این لینک مخصوص شما و همان همیار فنی است و تنها **تا ۷ روز اعتبار** دارد. پس از گذشت این مدت، بازخوردی که از طریق این لینک ثبت شود، به جایی ارسال نخواهد شد.
...
با این حال، در صورت بروز مسئله خاص یا نیاز به بررسی رسمی، **فقط افراد در سطح منتوری** امکان دسترسی مدیریتی به آن‌ها را خواهند داشت. توجه داشته باشید که برنامه بر مبنای اعتماد پیش می‌رود و هدف از بازخورد، اصلاح و رشد است، نه قضاوت یا برخورد رسمی.

بعد از ارسال بازخورد خود به همیارفنی، او این بازخورد را دریافت می‌کند و با خود اوست که با این بازخورد چکار می‌خواهد بکند و چه تغییری در روال برگزاری جلسات فنی خویش اعمال نماید.
...
احتمال بروز مشکل در عملکرد بات بسیار کم است، اما در صورت نیاز، می‌توانید موضوع را در گروه **CS Internship - Coordination** مطرح کنید تا راهنمایی لازم ارائه شود.

همچنین اگر پیشنهادی برای بهبود عملکرد این فرآیند یا تجربه کار با بات دارید، خوشحال می‌شویم با ما به اشتراک بگذارید.
...
- **نقش همیارفتی:** اینترن‌ها از استپ ۳ به بعد این نقش را خواهند گرفت. داشتن این نقش به معنی این هست که از این به بعد جلسات فنی برای اینترن‌های استپ پایین‌تر برگزار خواهند کرد.
...
- **نقش منتورفنی:** دستیارمنتورهایی که بر کیفیت عملکرد فنی اینترن‌های دوره مشخص نظارت و جلسات فنی برگزار می‌کنند، نقش منتور فنی را بر عهده دارند. این افراد در بالاترین سطح فنی دوره مشخص هستند و مسئولیت فنی دوره مشخص خودشان را برعهده دارند.
